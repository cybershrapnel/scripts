...
`;
  ...
  const recordOne = async (el) => {
    ...
    await wait(800);
    const lockedTitle = getTitle() || "suno";
    const id = trackId(el);
    log("locked title", lockedTitle, "dur", el.duration);

    await new Promise((resolve) => {
      const tick = setInterval(() => {
        if (!running) { clearInterval(tick); resolve(); return; }
        if (trackId(el) !== id) { clearInterval(tick); resolve(); return; }
        log("frames", frames, "t", el.currentTime.toFixed(1), "file=", lockedTitle);
      }, 2000);
  ...
  log("v1.2-bg title-lock. Filename is frozen 0.8s after play, not read again at save.");
})();
