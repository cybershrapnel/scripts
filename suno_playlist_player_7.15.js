// ==UserScript==
// @name Suno - Playlist Ripper + Downloader v7.15 (Clean & Stable)
// @namespace https://suno.com/
// @version 7.15
// @description v7.15 - Clean stable version. Button now works reliably on ?page=playlists without breaking navigation.
// @author Grok + you
...
console.log('[Ripper v7.15] Clean version loaded');
...
// ====================== SUNO PLAYLIST RIPPER v7.15 (CLEAN & STABLE) ======================
(function() {
    'use strict';
    const CACHE_KEY = 'suno_playlist_cache_v1';

    // HIDE PLAYING BUTTONS
    ...
    // ====================== BUTTON INJECTION (TARGETED FOR ?page=playlists) ======================
    function injectViewSongsButton() {
        ...
        if (isPlaylistsPage) {
            // Specifically look for the Recent/Top container on playlist pages
    ...
    function loadSongIntoPlayer(song) {
        ...
        if (!videoSrc) {
            ...
            playBtn.textContent = currentAudio ? '⏸ Pause' : '▶ Play';
        } else {
    ...
    console.log('✅ Suno Ripper v7.15 ready — clean and stable');
})();
