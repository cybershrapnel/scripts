...
#ncz-chat-mirror-host .ncz-replyBtn{
  ...
  /* NEW: pin to far-right of the message pill */
  ...
  function sendFromMirror() {
    ...
    setTimeout(() => {
      if (sendBtn && !sendBtn.disabled) {
        ...
        // still try click; some UIs flip disabled right after input event
        sendBtn.click();
      }
    }, 0);
...
function closestFlexRowByClass(el, stopEl) {
  let p = el;
  while (p && p !== stopEl) {
    const cls = typeof p.className === "string" ? p.className : "";
    if (/\bflex\b/.test(cls)) return p;
    p = p.parentElement;
  }
  // if stopEl itself is flex, use it; otherwise fallback to stopEl
  const rootCls = stopEl && typeof stopEl.className === "string" ? stopEl.className : "";
  return /\bflex\b/.test(rootCls) ? stopEl : stopEl;
}
  ...
  function renderNow() {
    ...
    for (const msg of msgNodes) {
      ...
      if (activeRoom > 0) {
        const c = String(rawContent || "").trimStart();
        if (!c.startsWith(roomPrefix)) continue;
      }
      ...
      if (nameEl && username) {
        ...
        // Reply button (NEW) -> inserts @username into the mirrored input
        ...
        rbtn.addEventListener("click", (e) => {
          ...
          // don’t spam duplicates if it already starts with the mention
...
// Ensure the cloned message container is the positioning anchor
c.style.position = "relative";
c.style.overflow = "visible";          // safety
c.style.paddingRight = "34px";         // keep text from running under the icon

// Put reply button at far-right of the pill
c.appendChild(rbtn);
  ...
  // Global observer to reattach if container changes
